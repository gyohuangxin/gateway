	...
	for _, httpListener := range httpListeners {
		...
		if httpListener.TLS != nil {
			for t := range httpListener.TLS.Certificates {
				secret := buildXdsDownstreamTLSSecret(httpListener.TLS.Certificates[t], httpListener.TLS.PrivateKeyProvider)
	...
	for _, tcpListener := range tcpListeners {
		...
		if tcpListener.TLS != nil && tcpListener.TLS.Terminate != nil {
			for _, s := range tcpListener.TLS.Terminate.Certificates {
				secret := buildXdsDownstreamTLSSecret(s, tcpListener.TLS.Terminate.PrivateKeyProvider)
				if err := tCtx.AddXdsResource(resourcev3.SecretType, secret); err != nil {
					errs = multierror.Append(errs, err)
				}
			}
